...
runComponentHydrate
  ...
  -> Effect (Ref (DriverStateX HTML r f o))
runComponentHydrate renderSpecWithHydration isRoot currentNode lchs handler j = Render.runComponentImplementation renderSpecWithHydration.renderSpec isRoot lchs handler j runRender
  where
    runRender :: DriverStateX HTML r f o -> Effect Unit
...
renderHydrate
  ...
  -> DOM.Node
  -> Ref LifecycleHandlers
  -> Ref (DriverState HTML r s f act ps i o)
  -> Effect Unit
renderHydrate renderSpecWithHydration isRoot currentNode lchs var = Render.renderImplementation renderSpecWithHydration.renderSpec isRoot lchs var runRender
  where
  runRender :: (Input act -> Aff Unit) -> (act -> Aff Unit) -> DriverStateRec HTML r s f act ps i o -> Effect (r s act ps o)
...
renderChildHydrate
  ...
  -> Ref LifecycleHandlers
  -> (act -> Aff Unit)
  -> Ref (Slot.SlotStorage ps (DriverStateRef HTML r))
  -> Ref (Slot.SlotStorage ps (DriverStateRef HTML r))
...
renderChildHydrate renderSpecWithHydration lchs handler childrenInRef childrenOutRef componentSlotBox currentNode = Render.renderChildImplementation renderSpecWithHydration.renderSpec lchs handler childrenInRef childrenOutRef renderWithExistingChildrenState renderNew componentSlotBox
  where
  renderWithExistingChildrenState :: forall query input output. ComponentSlotSpec HTML query input output ps Aff act -> Tuple (DriverStateRef HTML r query output) (Slot.SlotStorage ps (DriverStateRef HTML r)) -> Effect (Ref (DriverStateX HTML r query output))
  ...
  renderNew :: forall query input output. ComponentSlotSpec HTML query input output ps Aff act -> Effect (Ref (DriverStateX HTML r query output))
