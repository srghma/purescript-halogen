...
import Halogen.Component (Component, ComponentSlotBox, ComponentSlotSpec, unComponent, unComponentSlot)
...
render
  :: forall s f act ps i o r surface
   . RenderSpec surface r
  -> Boolean
...
render renderSpec isRoot lchs var = renderImplementation renderSpec isRoot lchs var runRender
  where
  runRender :: (Input act -> Aff Unit) -> (act -> Aff Unit) -> DriverStateRec surface r s f act ps i o -> Effect (r s act ps o)
...
renderChild
  :: forall ps act r surface
   . RenderSpec surface r
  -> Ref LifecycleHandlers
  -> (act -> Aff Unit)
  -> Ref (Slot.SlotStorage ps (DriverStateRef surface r))
  -> Ref (Slot.SlotStorage ps (DriverStateRef surface r))
  -> ComponentSlotBox surface ps Aff act
...
renderChild renderSpec lchs handler childrenInRef childrenOutRef = renderChildImplementation renderSpec lchs handler childrenInRef childrenOutRef renderWithExistingChildrenState renderNew
  where
  renderWithExistingChildrenState :: forall query input output. ComponentSlotSpec surface query input output ps Aff act -> Tuple (DriverStateRef surface r query output) (Slot.SlotStorage ps (DriverStateRef surface r)) -> Effect (Ref (DriverStateX surface r query output))
  ...
  renderNew :: forall query input output. ComponentSlotSpec surface query input output ps Aff act -> Effect (Ref (DriverStateX surface r query output))
  renderNew slot = runComponent renderSpec false lchs (maybe (pure unit) handler <<< slot.output) slot.input slot.component

runComponent
  :: forall f i o r surface
   . RenderSpec surface r
  -> Boolean
  -> Ref LifecycleHandlers
  -> (o -> Aff Unit)
  -> i
  -> Component surface f i o Aff
  -> Effect (Ref (DriverStateX surface r f o))
runComponent renderSpec isRoot lchs handler j = runComponentImplementation renderSpec isRoot lchs handler j runRender
  where
    runRender :: DriverStateX surface r f o -> Effect Unit
...
renderImplementation
  ...
  -> Ref LifecycleHandlers
  -> Ref (DriverState surface r s f act ps i o)
  -> ((Input act -> Aff Unit) -> (act -> Aff Unit) -> DriverStateRec surface r s f act ps i o -> Effect (r s act ps o))
  -> Effect Unit
renderImplementation renderSpec isRoot lchs var runRender = Ref.read var >>= \(DriverState ds) -> do
  ...
  let
    ...
    selfRef :: Ref (DriverState surface r s f act ps i o)
...
renderChildImplementation
  ...
  -> Ref (Slot.SlotStorage ps (DriverStateRef surface r))
  -> Ref (Slot.SlotStorage ps (DriverStateRef surface r))
  -> (forall query input output. ComponentSlotSpec surface query input output ps Aff act -> Tuple (DriverStateRef surface r query output) (Slot.SlotStorage ps (DriverStateRef surface r)) -> Effect (Ref (DriverStateX surface r query output)))
  -> (forall query input output. ComponentSlotSpec surface query input output ps Aff act -> Effect (Ref (DriverStateX surface r query output)))
  -> ComponentSlotBox surface ps Aff act
  -> Effect (RenderStateX r)
...
runComponentImplementation
  ...
  -> (o -> Aff Unit)
  -> i
  -> (DriverStateX surface r f o -> Effect Unit)
  -> Component surface f i o Aff
  -> Effect (Ref (DriverStateX surface r f o))
...
squashChildInitializers
  :: forall f o r surface
   . RenderSpec surface r
  -> Boolean
  -> Ref LifecycleHandlers
  -> L.List (Aff Unit)
  -> DriverStateX surface r f o
  -> Effect Unit
